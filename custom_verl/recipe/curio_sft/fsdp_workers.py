...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        if self._is_ref:
            self.use_adaptive_tau = self.config.actor.get("use_adaptive_tau",False)
            self.sync_ref_model = self.config.actor.get("sync_ref_model", False)
            self.ref_model_sync_steps = self.config.actor.get("ref_model_sync_steps", 1)
            self.ref_model_mixup_alpha = self.config.actor.get("ref_model_mixup_alpha", 0.0)
            ...
            print("==============")
            print("ref_model_sync_steps",self.ref_model_sync_steps)
            print("ref_model_mixup_alpha",self.ref_model_mixup_alpha)
    ...
    def _build_model_optimizer(
        ...
        if role == "actor" and optim_config is not None:
            ...
            total_steps = optim_config.get("total_training_steps", 0) * 4
    ...
    def init_model(self):
        from recipe.curio_sft.dp_actor import DataParallelPPOActor
        ...
        if self._is_ref:
            if self.role == "ref":
                local_path = copy_to_local(self.config.model.ref_path, use_shm=use_shm)
            else:
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with adapter_ctx:
                results = self.actor.compute_log_prob(data=data, calculate_entropy=True,use_adaptive_tau=self.use_adaptive_tau)
                if len(results) == 3:
                    output, entropys, extra_dict = results
                else:
                    output, entropys = results
            ...
            if len(results) == 3:
                results_dict["tau"] = extra_dict["tau"]
            output = DataProto.from_dict(
                tensors=results_dict,
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        if (
            ...
            print('='*100)
            print(f"Syncing ref model at step {self.ref_forward_step}")
            ...
            torch.distributed.barrier()
            print(f"Time taken to sync ref model: {time.time() - start_time}")
        # else:
        ...
        data.meta_info["temperature"] = self.config.ref.get("ref_temperature",1.0)
        data.meta_info["max_token_len"] = self.config.ref.log_prob_max_token_len_per_gpu
        data.meta_info["use_dynamic_bsz"] = self.config.ref.log_prob_use_dynamic_bsz

        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            results = self.ref_policy.compute_log_prob(data=data, calculate_entropy=False,use_adaptive_tau=self.use_adaptive_tau)
            if len(results) == 3:
                output, entropys, extra_dict = results
                output = DataProto.from_dict(tensors={"ref_log_prob": output,"tau":extra_dict["tau"]})
            else:
                output, entropys = results
