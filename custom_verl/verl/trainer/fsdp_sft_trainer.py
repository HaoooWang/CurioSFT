...
class FSDPSFTTrainer:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        entropy_prompt = torch.tensor(0.0)
        entropy_answer = torch.tensor(0.0)
        entropy_all = torch.tensor(0.0)
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                # entropy = self._calculate_entropy(shift_logits)
                # entropy_all = entropy.mean()
                # entropy_prompt = torch.masked_select(entropy, ~loss_mask)
                # entropy_answer = torch.masked_select(entropy, loss_mask)
                
                entropy_rmpad = self.compute_entropy_from_logits(shift_logits)
                loss_mask_2d = loss_mask.view(input_ids.shape[0], -1)  # 重新reshape为(batch_size, seqlen)
                seq_positions = torch.arange(loss_mask_2d.size(1), device=loss_mask_2d.device).unsqueeze(0).expand_as(loss_mask_2d)
                
                first_response_positions = torch.where(
                    loss_mask_2d.any(dim=1, keepdim=True),  # 检查是否有response
                    torch.argmax((loss_mask_2d == 1).int(), dim=1, keepdim=True),
                    torch.full((input_ids.shape[0], 1), loss_mask_2d.size(1), device=loss_mask_2d.device)
                )
                
                prompt_mask = seq_positions < first_response_positions
                prompt_mask = prompt_mask.view(-1)  # reshape回原来的形状
                
                entropy_prompt = torch.masked_select(entropy_rmpad, prompt_mask.bool()).mean().detach().item()
                entropy_answer = torch.masked_select(entropy_rmpad, loss_mask.bool()).mean().detach().item()
                entropy_all = entropy_rmpad.mean().detach().item()
                ...
                entropy_padded = pad_input(
                    hidden_states=entropy.unsqueeze(-1),
                    indices=indices,
                    batch=batch_size,
                    seqlen=seqlen,
                )
                
                # 计算prompt_mask：每个数据第一个loss_mask不为1的token之前的都是prompt
                # loss_mask形状: (batch_size * seqlen,) 已经reshape过
                loss_mask_2d = loss_mask.view(batch_size, -1)  # 重新reshape为(batch_size, seqlen)
                
                # 向量化计算：找到每行第一个loss_mask为1的位置
                # 创建位置索引
                seq_positions = torch.arange(loss_mask_2d.size(1), device=loss_mask_2d.device).unsqueeze(0).expand_as(loss_mask_2d)
                ...
                first_response_positions = torch.where(
                    loss_mask_2d.any(dim=1, keepdim=True),  # 检查是否有response
                    torch.argmax((loss_mask_2d == 1).int(), dim=1, keepdim=True),
                    torch.full((batch_size, 1), loss_mask_2d.size(1), device=loss_mask_2d.device)
                )
                
                prompt_mask = seq_positions < first_response_positions
                prompt_mask = prompt_mask.view(-1)  # reshape回原来的形状
                
                entropy_prompt = torch.masked_select(entropy_padded, prompt_mask.bool()).mean().detach().item()
                entropy_answer = torch.masked_select(entropy_padded, loss_mask.bool()).mean().detach().item()
                entropy_all = entropy.mean().detach().item()
            ...
            if do_backward:
                loss.backward()
            return loss,{'entropy_prompt': entropy_prompt, 'entropy_answer': entropy_answer, 'entropy_all': entropy_all}
    ...
    def training_step(self, batch: TensorDict):
        ...
        # print('!!!!!!!!!!!!!!!batch.shape!!!!!!!!!!')
        # print(batch['input_ids'].shape) # 32,10000
        micro_batches = batch.split(self.config.data.micro_batch_size_per_gpu)
        n_micro_batches = len(micro_batches)
        step_loss = 0
        all_metrics = {'entropy_prompt': [], 'entropy_answer': [], 'entropy_all': []}
        for micro_batch in micro_batches:
            loss,metrics = self._compute_loss_and_backward(batch=micro_batch)
            loss = loss / n_micro_batches
            for k,v in metrics.items():
                all_metrics[k] = all_metrics[k] + [v]
            step_loss += loss.item()
        ...
        for k,v in all_metrics.items():
            all_metrics[k] = torch.tensor(v).float().mean().detach().item()
        return {"train/loss": step_loss.detach().item(), "train/lr(1e-3)": lr * 1e3, **all_metrics}

    def validation_step(self, batch: TensorDict):
        self.fsdp_model.eval()
        with torch.no_grad():
            loss,metrics = self._compute_loss_and_backward(batch, do_backward=False)
            if is_cuda_available:
